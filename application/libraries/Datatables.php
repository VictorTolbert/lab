...
class Datatables
{
    ...
    /**
       * Generates WHERE $key_condition NOT IN $val
       * @param string $key_condition key to check
       * @param array $val values of key you don't want included
       * @return \Datatables
       */
    public function where_not_in($key_condition, $val = null)
    {
        $this->where_not_in[] = [$key_condition, $val];
    ...
    private function get_ordering()
    {
        ...
        $intvalisort = intval($this->ci->input->post('iSortingCols'));
        for ($i = 0; $i < $intvalisort; $i++) {
        ...
        for ($i = 0; $i < intval($this->ci->input->post('iColumns')); $i++) {
            if (isset($_POST['sSearch_' . $i]) && $this->ci->input->post('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns)) {
                ...
                foreach ($miSearch as $val) {
                    ...
                    } elseif (preg_match("/~/", trim($val))) { // column filter range separator
                        $clean_val = preg_split("/~/", trim($val));
                        if (! empty($clean_val[0])) { // if tilde last, <
                            $clean_val[0] = preg_match("/[0-9]{1,2}\/[0-9]{1,2}\/[0-9]{2}\s+[0-9]{1,2}:/", $clean_val[0]) ? strtotime($clean_val[0]) : $clean_val[0];
                            $this->ci->db->where($this->select[$mColArray[$i]] . ' >', $clean_val[0]);
                        }

                        if (! empty($clean_val[1])) { // if tilde first, >
                            $clean_val[1] = preg_match("/[0-9]{1,2}\/[0-9]{1,2}\/[0-9]{2}\s+[0-9]{1,2}:/", $clean_val[1]) ? strtotime($clean_val[1]) : $clean_val[1];
                            $this->ci->db->where($this->select[$mColArray[$i]] . ' <', $clean_val[1]);
                        }
                    } else {
    ...
    /**
       * Returns the users totals optimized
       *
       * @param type $filtering
       * @return int
       */
    private function get_total_users($filtering = false)
    {
    ...
    {
        if ('users' == $this->table) {
            return $this->get_total_users($filtering);
        }
    ...
    private function explode($delimiter, $str, $open = '(', $close=')')
    {
        ...
        $hold    = [];
        $balance = 0;
        $parts   = is_array($str) ? $str : explode($delimiter, $str);
