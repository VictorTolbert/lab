...
abstract class REST_Controller extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        if (! $this->input->is_ajax_request() and config_item('rest_ajax_only')) {
            $this->response(['status' => false, 'error' => 'Only AJAX requests are accepted.'], 505);
        }
    }
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (config_item('force_https') and ! $this->_detect_ssl()) {
            $this->response(['status' => false, 'error' => 'Unsupported protocol'], 403);
        }
        ...
        if (config_item('rest_enable_keys') and $use_key and $this->_allow === false) {
            ...
            $this->response(['status' => false, 'error' => 'Invalid API Key.'], 403);
        }

        // Sure it exists, but can they do anything with it?
        if (! method_exists($this, $controller_method)) {
            $this->response(['status' => false, 'error' => 'Unknown method.'], 404);
        }
        ...
        if (config_item('rest_enable_keys') and ! empty($this->rest->key)) {
            ...
            if (config_item('rest_enable_limits') and ! $this->_check_limit($controller_method)) {
                $this->response(['status' => false, 'error' => 'This API key has reached the hourly limit for this method.'], 401);
            }
            ...
            $authorized or $this->response(['status' => false, 'error' => 'This API key does not have enough permissions.'], 401);
    ...
    public function response($data = [], $http_code = null)
    {
        ...
        if (empty($data) && $http_code === null) {
            ...
            $output = null;
        } else { // Otherwise (if no data but 200 provided) or some data, carry on camping!
            ...
            } elseif (method_exists($this->format, 'to_' . $this->response->format)) { // If the format method exists, call and return the output in that format
                // Set the correct format header
                header('Content-Type: ' . $this->_supported_formats[$this->response->format]);
    ...
    protected function _detect_input_format()
    {
        if ($this->input->server('CONTENT_TYPE')) {
            ...
            foreach ($this->_supported_formats as $format => $mime) {
                $match = $this->input->server('CONTENT_TYPE');
                if (strpos($match, ';')) {
    ...
    protected function _detect_api_key()
    {
        ...
        if ($key) {
            $this->client = $this->rest->db->where(config_item('rest_key_column'), $key)->get(config_item('rest_keys_table'))->row();
            if (! $this->client) {
                return false;
            }
    ...
    protected function _parse_put()
    {
        ...
        } else { // If no file type is provided, this is probably just arguments
    ...
    protected function _prepare_basic_auth()
    {
        ...
        } elseif ($this->input->server('HTTP_AUTHENTICATION')) { // most other servers
